            ...
            {emailConfirmSent ? (
              ...
              >
                ...
                <div className="w-20 h-20 rounded-2xl bg-[#6E64FF]/10 border border-[#6E64FF]/20 flex items-center justify-center">
                  <svg className="w-10 h-10 text-[#6E64FF]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="space-y-2">
                  <h2 className="text-2xl font-bold tracking-tight text-white">{t('auth.confirm.title')}</h2>
                  <p className="text-sm text-white/50 leading-relaxed max-w-xs mx-auto">
                    {t('auth.confirm.desc')}{' '}
                    <span className="text-white font-semibold">{email}</span>。
                    <br />
                    {t('auth.confirm.desc2')}
                  </p>
                </div>
              ...
              <div className="flex items-center gap-3 py-2">
                <button 
                  ...
                  className={`w-5 h-5 rounded border transition-all flex items-center justify-center ${agreeTerms ? 'bg-brand-primary border-brand-primary' : 'border-white/20 bg-[#0A051A]'}`}
                >
                  {agreeTerms && (
                ...
                <span className="text-[11px] text-white/40">
                  {t('auth.signup.agree_terms')} <Link to="/terms" className="underline hover:text-white/60">{t('auth.signup.terms_link')}</Link>
                </span>
              </div>
              ...
              >
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : t('auth.signup.submit')}
              </button>

              <div className="pt-4 text-center">
                <p className="text-xs text-white/40 font-medium">
                  {t('auth.signup.has_account')}{' '}
                  <Link to="/login" className="text-white/60 hover:text-white underline underline-offset-4 ml-1">{t('auth.signup.sign_in')}</Link>
                </p>
              </div>
            </form>
            )}
